...
export default function MapScreen(props) {
    ...
    const onPressBook = async () => {
        ...
        if (contains([tripdata.pickup.lat, tripdata.pickup.lng], restrictedArry) && contains([tripdata.drop.lat, tripdata.drop.lng], restrictedArry)) {
            // console.log("IN");
            ...
            // console.log("OUT");
            Alert.alert(t('alert'), t('no_service_area_alert_messege'));
            setBookLoading(false);
        }
    }
    ...
    return (
        <View style={styles.container}>
            ...
            />
            <StatusBar hidden={true} backgroundColor={colors.PINK.primary} />
            ...
            <View style={styles.buttonBar}>
                <Button
                    ...
                    buttonStyle={[styles.buttonStyle, { backgroundColor: colors.BLUE.primary }]}
            ...
            <View style={styles.menuIcon}>
                <TouchableOpacity onPress={() => { props.navigation.toggleDrawer() }} style={styles.menuIconButton} >
                    ...
                    <Image
                        ...
                        style={{ width: 75, height: 60, marginTop: 0 }}
                    />
                </TouchableOpacity>
            </View>
            {activeBookings && activeBookings.length >= 1 ?
                <View style={styles.topTitle}>
                    <TouchableOpacity onPress={() => {
                        ...
                        <Text style={{ marginHorizontal: 7, textAlign: 'center', color: colors.BLUE.primary, fontFamily: 'Poppins-Bold', fontSize: 14 }}>{t('active_booking')} - {activeBookings.length} </Text>

                    </TouchableOpacity>
                </View>
                :
                <View style={[styles.topTitle, { width: 100 }]}>
                    <Text style={{ marginHorizontal: 7, textAlign: 'center', color: colors.BLUE.primary, fontFamily: 'Poppins-Bold', fontSize: 18, }}>{t('home')}</Text>
                </View>
            }
            <View style={styles.addressBar}>
                ...
                <View style={styles.contentStyle}>
                    <TouchableOpacity onPress={() => tapAddress('pickup')} style={styles.addressStyle1}>
                        <Text numberOfLines={1} style={[styles.textStyle, tripdata.selected == 'pickup' ? { fontSize: 16 } : { fontSize: 14, color: colors.GREY.Trolley_Grey }]}>{tripdata.pickup && tripdata.pickup.add ? tripdata.pickup.add : t('map_screen_where_input_text')}</Text>
                    </TouchableOpacity>
                    <View style={{ height: 1, marginRight: 15, backgroundColor: colors.GREY.btnPrimary }} />
                    <TouchableOpacity onPress={() => tapAddress('drop')} style={styles.addressStyle2}>
                        <Text numberOfLines={1} style={[styles.textStyle, tripdata.selected == 'drop' ? { fontSize: 16 } : { fontSize: 14, color: colors.GREY.Trolley_Grey }]}>{tripdata.drop && tripdata.drop.add ? tripdata.drop.add : t('map_screen_drop_input_text')}</Text>
                    </TouchableOpacity>
                </View>
            </View>
                    ...
                    {allCarTypes.map((prop, key) => {
                        return (
                            <View key={key}>
                                ...
                                <View style={[styles.cabDivStyle, { backgroundColor: prop.active == true ? colors.YELLOW.secondary : colors.WHITE }]}>
                                    <View style={[{
                                        backgroundColor: prop.active == true ? colors.BLUE.primary : colors.GREY.Deep_Nobel,
                                        width: 28, height: 28, borderRadius: 50, justifyContent: 'center',
                                        position: 'absolute',
                                        top: -12,
                                        zIndex: 999
                                    }]}>
                                        {/* <Ionicons name="person-outline" size={18} color="white" style={{ alignSelf: 'center' }} /> */}
                                        <Image resizeMode="contain" source={require('../../assets/images/ic_man_driver.png')} style={{ width: 18, height: 18, alignSelf: 'center' }} />
                                    </View>
                                    ...
                                    <View style={styles.textViewStyle}>
                                        ...
                                        <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 0 }}>
                                            {/* <Text style={[styles.text2, { fontWeight: 'bold', color: colors.GREY.btnPrimary }]}>
                                            {settings.symbol}{prop.rate_per_unit_distance} / {settings.convert_to_mile ? t('mile') : t('km')}
                                        </Text> */}
                                            {
