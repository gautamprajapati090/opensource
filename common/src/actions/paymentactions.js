            ...
            walletBalRef(uid).set(walletBalance).then(() => {
                walletHistoryRef(uid).push(details).then(()=>{
                    dispatch({
                        ...
                        payload: store.getState().languagedata.defaultLanguage.wallet_updated
                    });
                }).catch(error=>{
                    dispatch({
                ...
                RequestPushMsg(snapshot.val().pushToken, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.wallet_updated)(firebase);
            }).catch(error=>{
                dispatch({
                    type: UPDATE_WALLET_BALANCE_FAILED,
                    payload: error.code + ": " + error.message,
