...
export const updateBooking = (booking) => (dispatch) => async (firebase) => {
  ...
  if (booking.status == 'ARRIVED') {
    ...
    singleBookingRef(booking.id).update(booking);
    RequestPushMsg(booking.customer_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.driver_near)(firebase);
  }
  if (booking.status == 'STARTED') {
    ...
    RequestPushMsg(booking.customer_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.driver_journey_msg + booking.id)(firebase);
  }
  if (booking.status == 'REACHED') {
    ...
    if(appcat == 'taxi'){
      ...
      const fare = FareCalculator(distance, totalTimeTaken, rates, null);
      booking.drop = { 
        add: address,
        lat: driverLocation.lat,
        lng: driverLocation.lng
    ...
    singleBookingRef(booking.id).update(booking);
    RequestPushMsg(booking.customer_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.driver_completed_ride)(firebase);
  }
  ...
  if (booking.status == 'PAID') {
    ...
    singleUserRef(booking.driver).once('value', snapshot => {
      ...
      walletBalRef(booking.driver).set(parseFloat(walletBalance.toFixed(2)));

      let details = {
        type: 'Credit',
        amount: parseFloat(booking.driver_share).toFixed(2),
    ...
    RequestPushMsg(booking.customer_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.success_payment)(firebase);
    RequestPushMsg(booking.driver_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.success_payment)(firebase);
  }
  if (booking.status == 'COMPLETE') {
    singleBookingRef(booking.id).update(booking);
    if (booking.rating) {
      RequestPushMsg(booking.driver_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.received_rating.toString().replace("X", booking.rating.toString()))(firebase);
      singleUserRef(booking.driver).once('value', snapshot => {
        let profile = snapshot.val();
        let ratings = {};
...
export const cancelBooking = (data) => (dispatch) => (firebase) => {
  ...
  singleBookingRef(data.booking.id).update({
    ...
    if (data.booking.driver && (data.booking.status === 'ACCEPTED' || data.booking.status === 'ARRIVED')) {
      ...
      RequestPushMsg(data.booking.driver_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.booking_cancelled + data.booking.id)(firebase);
      RequestPushMsg(data.booking.customer_token, store.getState().languagedata.defaultLanguage.notification_title, store.getState().languagedata.defaultLanguage.booking_cancelled + data.booking.id)(firebase);
    }
