...
export const sendMessage = (data) => (dispatch) => (firebase) => {
    ...
    if(data.role == 'rider'){
        RequestPushMsg(data.booking.driver_token, store.getState().languagedata.defaultLanguage.notification_title + store.getState().languagedata.defaultLanguage.chat_requested, data.message)(firebase);
    }else{
        RequestPushMsg(data.booking.customer_token, store.getState().languagedata.defaultLanguage.notification_title + store.getState().languagedata.defaultLanguage.chat_requested, data.message)(firebase);
    }
